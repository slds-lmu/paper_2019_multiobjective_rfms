...
foldername = "../output/"
if (!exists(c("context", "list_res", "foldername"))) stop("require definition of variable 'context' and 'list_res', mean_hypervolumes and win-loss plot could not be executed alone!")
pdf(sprintf(paste0(foldername, "wins_and_losses_%s.pdf"), context), width = 8, height = 6)
...
pdf(file = sprintf(paste0(foldername, "mean_hypervolumes_%s.pdf"), context), height = 7, width = 10)
...
pdf(file = sprintf(paste0(foldername, "hypervolumes_%s.pdf"), context), height = 7, width = 10)
