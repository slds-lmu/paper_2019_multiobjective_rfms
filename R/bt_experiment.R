...
#' @param prob_inputs list of anytype
...
#' @examples
#' x=c(1,2,3)
init = function(prob_names, prob_inputs, prob_funs, algo_names, algo_funs, reg_input, algo_designs, repls) {
  for (pname in prob_names) {
    addProblem(name = pname,  data = prob_inputs, fun = prob_funs[[pname]], seed = 1L, reg = reg_input)
  }
